...
bang_func_cpp_src = """
torch::Tensor bang_func(torch::Tensor x, torch::Tensor kernel, int in_channels, int out_channels, int kernel_size, int groups);
"""

bang_func_ext = load_inline(
    name="Grouped_conv_2D",
...
class ModelNew(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return bang_func_ext.bang_func(x, self.kernel, self.in_channels, self.out_channels, self.kernel_size, self.groups)

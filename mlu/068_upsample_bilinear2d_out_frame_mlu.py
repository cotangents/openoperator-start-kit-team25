...
bang_func_cpp_src = """
torch::Tensor bang_func(torch::Tensor input, int out_h, int out_w, bool align_corners);
"""

bang_func_ext = load_inline(
    name="upsample_bilinear2d_out_frame",
...
class ModelNew(nn.Module):
    ...
    def forward(self, input: torch.Tensor, out_h: int, out_w: int, align_corners: bool = False) -> torch.Tensor:
        return bang_func_ext.bang_func(input, out_h, out_w, align_corners)